...
public class ALIyerEdonAssets_Lighting : EditorWindow
{
    ...
    void OnEnable()
    {
        ...
        if (EditorPrefs.GetInt("dontShow_Glass_Pack") == 3)
            dontShow = true;
        if (EditorPrefs.GetInt("dontShow_Glass_Pack") != 3)
            dontShow = false;
    ...
    private void OnGUI()
    {
        ...
        {
            if (dontShow == true)            
                EditorPrefs.SetInt("dontShow_Glass_Pack", 3); // 3 == true
            if (dontShow == false)
                EditorPrefs.SetInt("dontShow_Glass_Pack", 0); // 0 = false
        }
...
public class Startup
{
    static Startup() 
    {
        EditorPrefs.SetInt("showCounts_Glass_Pack", EditorPrefs.GetInt("showCounts_Glass_Pack") + 1);
        if (EditorPrefs.GetInt("showCounts_Glass_Pack") < 2)
        { 
        ...
        {   
            if (EditorPrefs.GetInt("dontShow_Glass_Pack") == 3)
            {
                if (EditorPrefs.GetInt("showCounts_Glass_Pack") >= 210)
                    EditorPrefs.SetInt("showCounts_Glass_Pack", 0);
            }
            else
            {
                if (EditorPrefs.GetInt("showCounts_Glass_Pack") >= 50)
                    EditorPrefs.SetInt("showCounts_Glass_Pack", 0);
            }
        }            
    }
